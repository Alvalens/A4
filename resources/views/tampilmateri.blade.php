...
@section('content')
        ...
        <form action="{{ route('materials.update', $materi->id) }}" method="POST">
          ...
          <div class="modal-body">
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="judul" name="judul" required placeholder="Judul Materi"
                value="{{ old('judul') ?? $materi->judul }}">
            </div>
            <div class="mb-3">
              <label for="level" class="form-label">Level</label>
              <input type="text" class="form-control" id="level" name="level" required placeholder="Level Materi"
                value="{{ old('level') ?? $materi->level }}">
            </div>
            <div class="mb-3">
              <label for="deskripsi" class="form-label">Deskripsi</label>
              <input type="text" class="form-control" id="deskripsi" name="deskripsi" placeholder="Deskripsi Materi (Opsional)"
                value="{{ old('deskripsi') ?? $materi->deskripsi }}">
            </div>
            <div class="mb-3">
              <label for="link" class="form-label">Link</label>
              <input type="url" class="form-control" id="link" name="link" required placeholder="https://www.youtube.com/embed/..."
  ...
  <div class="modal fade" id="fileModalDelete" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="fileModalDeleteLabel">Hapus Materi</h5>
          <hr>
          <p>Apa kamu yakin ingin menghapus materi ini?</p>
        </div>
        <div class="modal-footer">
